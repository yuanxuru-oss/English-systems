...
export function createAudioPlayer(options = {}) {
  ...
  if (nativeAudio) {
    ...
    nativeAudio.addEventListener("error", () => {
      console.warn("Audio element error — source may be invalid");
      timeCurrent.textContent = "⚠️";
      timeCurrent.style.color = "var(--rose)";
    ...
    nativeAudio.addEventListener("loadedmetadata", () => {
      ...
      container.classList.remove("audio-error");
    });
  }
    ...
    <div class="audio-player-inner">
      ...
      <div class="audio-player-track">
        <div class="audio-progress-bar" data-audio-progress-bar>
          ...
          <img class="audio-progress-thumb" data-audio-progress-thumb style="left:0%" src="${window.fishboneDataUri || ''}" alt="" />
        </div>
        <div class="audio-player-info">
          <span class="audio-label">${label}${!hasAudio ? ' <small style="color:var(--muted)">(无音频文件)</small>' : ''}</span>
