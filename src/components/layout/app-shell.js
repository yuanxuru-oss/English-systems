window.fishboneDataUri = "data:image/png;base64,";
import { icon } from "../icons.js";
import { APP_META } from "../../config/app-meta.js";

function navItem(route, label, iconName, activeRoute) {
  return `
    <button data-route="${route}" class="nav-pill ${activeRoute === route ? "is-active" : ""}" type="button">
      ${icon(iconName)}
      <span>${label}</span>
    </button>
  `;
}

function navGroup(label, iconName, routes, activeRoute) {
  const isOpen = routes.some(({ route }) => route === activeRoute);
  return `
    <details class="nav-group" ${isOpen ? "open" : ""}>
      <summary>
        ${icon(iconName)}
        <span>${label}</span>
        <span class="nav-group-arrow">⌄</span>
      </summary>
      <div class="nav-sublist">
        ${routes.map(({ route, label, iconName }) => navItem(route, label, iconName, activeRoute)).join("")}
      </div>
    </details>
  `;
}

export function createAppShell(store, navigate) {
  const state = store.getState();
  const project = state.projects.find((item) => item.id === state.currentProjectId) ?? state.projects[0];
  const activeRoute = state.route || "dashboard";

  const shell = document.createElement("div");
  shell.className = "app-shell";
  shell.innerHTML = `
    <aside class="sidebar">
      <div class="paper-noise"></div>
      <button class="sidebar-close" type="button" aria-label="关闭导航菜单">${icon("close")}</button>
      <div class="brand-block">
        <p class="brand-kicker">REVIEW ATELIER</p>
        <h1>鱼骨头英语复习系统</h1>
        <p class="brand-copy">把英语啃的只剩鱼骨头。</p>
      </div>
      <div class="sidebar-sketch">
        <div class="sketch-badge">${icon("user")} ${state.userName || "我的"}学习空间</div>
        <div class="sketch-profile-stats">
          <div class="sketch-stat">
            <span class="sketch-stat-num">${state.studyLog ? new Set(state.studyLog.map(e => e.date.slice(0,10))).size : 0}</span>
            <span class="sketch-stat-label">学习天数</span>
          </div>
          <div class="sketch-stat">
            <span class="sketch-stat-num">${state.flashcards ? state.flashcards.filter(c => c.mastered).length : 0}/${state.flashcards?.length || 0}</span>
            <span class="sketch-stat-label">已掌握闪卡</span>
          </div>
          <div class="sketch-stat">
            <span class="sketch-stat-num">${state.mistakes?.length || 0}</span>
            <span class="sketch-stat-label">错题积累</span>
          </div>
          <div class="sketch-stat">
            <span class="sketch-stat-num ${state.checkin?.isCheckedIn ? 'is-checked' : ''}">${state.checkin?.isCheckedIn ? '✓' : '—'}</span>
            <span class="sketch-stat-label">今日打卡</span>
          </div>
        </div>
      </div>
      <nav class="nav-list">
        ${navItem("dashboard", "首页", "dashboard", activeRoute)}
        ${navItem("project", "项目总览", "project", activeRoute)}
        ${navItem("import", "导入中心", "import", activeRoute)}
        ${navItem("listening", "听力练习", "listening", activeRoute)}
        ${navItem("reading", "阅读练习", "reading", activeRoute)}
        ${navItem("translation", "翻译练习", "translation", activeRoute)}
        ${navGroup("复习中心", "flashcards", [
          { route: "vocabulary", label: "词汇表", iconName: "flashcards" },
          { route: "mistakes", label: "错题笔记", iconName: "mistakes" },
          { route: "flashcards", label: "闪卡词库", iconName: "flashcards" },
        ], activeRoute)}
        ${navGroup("个人中心", "user", [
          { route: "profile", label: "个人资料", iconName: "user" },
          { route: "checkin", label: "打卡日历", iconName: "checkin" },
          { route: "settings", label: "设置", iconName: "gear" },
          { route: "obsidian", label: "Obsidian", iconName: "obsidian" },
        ], activeRoute)}
      </nav>
      <div class="project-chip">
        ${icon("pin")}
        <div>
          <span>当前项目</span>
          <strong>${project?.title ?? "未选择项目"}</strong>
        </div>
      </div>
      <div class="app-info-card">
        <div class="app-info-card-title">
          <span>鱼骨头英语复习</span>
          <strong>桌面版 v${APP_META.version}</strong>
        </div>
        <p>你的学习记录仅保存在这台设备上。</p>
      </div>
    </aside>
    <main class="main-panel">
      <header class="topbar">
        <button class="sidebar-toggle" type="button" aria-label="打开导航菜单" aria-expanded="false">${icon("menu")}</button>
        <div>
          <h2>${project?.title ?? "英语复习"}</h2>
        </div>
        <div class="checkin-state ${state.checkin.isCheckedIn ? "is-checked" : ""}">
          ${state.checkin.isCheckedIn
            ? '<span class="fishbone-stamp"><img src="' + fishboneDataUri + '" alt="打卡印章" /></span> 今日已打卡'
            : "今日未打卡"}
        </div>
      </header>
      <section data-shell-content></section>
    </main>
  `;

  shell.querySelectorAll("[data-route]").forEach((button) => {
    button.addEventListener("click", () => navigate(button.dataset.route));
  });

  const closeSidebar = () => {
    shell.classList.remove("is-sidebar-open");
    shell.querySelector(".sidebar-toggle")?.setAttribute("aria-expanded", "false");
  };
  shell.querySelector(".sidebar-toggle")?.addEventListener("click", () => {
    const isOpen = shell.classList.toggle("is-sidebar-open");
    shell.querySelector(".sidebar-toggle")?.setAttribute("aria-expanded", String(isOpen));
  });
  shell.querySelector(".sidebar-close")?.addEventListener("click", closeSidebar);

  return shell;
}
